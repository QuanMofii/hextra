...
- **単一設定** — プロジェクト名、GitHub URL、ロゴ、メニューは `hugo.yaml` で管理（[サイト設定](/guide/site-configuration/) 参照）。
- **プレースホルダー** — `PROJECT_NAME`、`your-username`、`your-project`、`{author}` / `{project_name}` を実際の値に置き換えてください（同上）。

## クイックスタート

1. GitHub とプロジェクトのプレースホルダーを置換（[GitHub URLプレースホルダーの置換](/guide/site-configuration/#github-urlプレースホルダーの置換) 参照）。
...
完全なチェックリストは [サイト設定](/guide/site-configuration/) を参照してください。
