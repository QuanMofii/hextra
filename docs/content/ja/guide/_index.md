---
title: ガイド
weight: 1
sidebar:
  open: true
---

PROJECT_NAME の使い方を学ぶには、以下のセクションを参照してください:

<!--more-->

{{< cards >}}
  {{< card link="organize-files" title="ファイルの整理" icon="document-duplicate" >}}
  {{< card link="theme-configuration" title="テーマ設定" icon="adjustments" >}}
  {{< card link="markdown" title="Markdown" icon="markdown" >}}
  {{< card link="syntax-highlighting" title="シンタックスハイライト" icon="sparkles" >}}
  {{< card link="latex" title="LaTeX" icon="variable" >}}
  {{< card link="diagrams" title="ダイアグラム" icon="chart-square-bar" >}}
  {{< card link="shortcodes" title="ショートコード" icon="template" >}}
  {{< card link="site-deploy" title="サイトデプロイ" icon="server" >}}
  {{< card link="site-configuration" title="サイト設定" icon="cog" >}}
  {{< card link="content-format" title="コンテンツフォーマット標準" icon="document-text" >}}
{{< /cards >}}